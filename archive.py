...
class Support_Vector_Machine(Regression_Model):
    def train(self, epoch, batch_size, reg, learning_rate):
        self.reg = reg
    ...
    def cost_function(self, X, Y):
        cost = 1/len(X) * sum([(abs(sum([self.weight[i+1]**2 for i in range(len(self.parameter_count))]))**2)/2 + self.reg*self.hinge_loss(X[n], Y[n]) for n in range(len(X))])
        self.cost += [cost]
        return cost

    def gradient_descent(self, X, Y):
        delta_weight = 1/len(X) * sum([self.weight if self.constraint == 0 else [self.weight[i+1]-X[n][i]*(self.reg*Y[n]) for i in range(self.parameter_count)] for n in range(len(X))])
        for j in range(self.parameter_count):
            self.weight[j+1] -= self.learning_rate*delta_weight
